...
const createProduct = async (req, res) => {
  const { title, price, size, description, category, brand, count } = req.body;
  let newFileName;
  if (req.file !== undefined) {
    let fileType = req.file.mimetype.split("/")[1];
    newFileName = `${req.user._id}${Date.now()}.${fileType}`;
    // fs.rename(
    //   `../client/public/images/products/${req.file.filename}`,
    //   `../client/public/images/products/${newFileName}`,
    //   function () {
    ...
    fs.rename(
      ...
      `./public/images/products/${newFileName}`,
      function () {
        console.log("Callback");
      }
    );
  ...
  try {
    const newProduct = await Product.create({
      title,
      price,
      img: `/images/products/${newFileName}`,
      size,
      description,
      category,
      brand,
